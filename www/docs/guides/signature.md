...
const myLedgerSigner = new LedgerSigner230(myLedgerTransport, 0);
...
A Node script : [here](https://github.com/PhilippeR26/starknet.js-workshop-typescript/blob/main/src/scripts/ledgerNano/10.testLedger230-rpc08.ts).  
...
If you want to use the accounts created with the v1.1.1, using the v2.3.0 :

```typescript
const myLedgerSigner = new LedgerSigner230(myLedgerTransport, 0, undefined, getLedgerPathBuffer111);
```
