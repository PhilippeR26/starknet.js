...
:::note
The last version of the Ledger Starknet APP (v2.2.5) supports explained V1 (ETH) & V3 (STRK) transactions & deploy accounts. For a class declaration or a message, you will have to blind sign a hash ; sign only hashes from a code that you trust. Do not forget to enable `Blind signing` in the APP settings.
:::
...
A Node script : [here](https://github.com/PhilippeR26/starknet.js-workshop-typescript/blob/main/src/scripts/ledgerNano/10.testLedger225-rpc08.ts).  
...
If you want to use the accounts created with the v1.1.1, using the v2.2.5 :

```typescript
const myLedgerSigner = new LedgerSigner225(myLedgerTransport, 0, undefined, getLedgerPathBuffer111);
```
